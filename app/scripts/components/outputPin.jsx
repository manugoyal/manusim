...
var Component = React.createClass({
    ...
    render: function() {
        ...
        return (
            <svg ref='svg' height={positions.height}
                 width={positions.width}
                 style={{position: 'fixed', left: obj.position.x,
                         top: obj.position.y}}
                 onMouseDown={this.handleMouseDown}>
                <rect x={tick.length/2} y={0}
                      width={positions.width - tick.length/2}
                      height={positions.height} fill='transparent'
                      stroke='black' strokeWidth={2} />
                <text x={tick.length+5} y={positions.height/2} fill='black'>
                    {_.isUndefined(inputValue) ?
                     'undefined' : inputValue.join('')}
                </text>
                <tick.Tick pos={positions.tickCoords[port.input][0]}
                           ref='input_0' />
            </svg>
        );
    }
